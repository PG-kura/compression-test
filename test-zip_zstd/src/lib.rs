...
pub fn archive<P>(input: common::Origin, path: P, level: i32) -> Result<common::Archive>
where
    P: AsRef<Path>,
{
    ...
    let zstd_file = fs::File::create(&path).unwrap();
    let mut encoder = zstd::stream::write::Encoder::new(zstd_file, level)?;
    encoder.write_all(&zip_buff)?;
    let zstd_file = encoder.finish()?;
    drop(zstd_file);
...
pub fn extract(archive: common::Archive) -> Result<common::Origin> {
    let zstd_file = fs::File::open(archive.path).unwrap();

    let mut decoder = zstd::stream::read::Decoder::new(&zstd_file)?;
    let mut zip_buff = vec![];
